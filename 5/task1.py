...
for instruction in instructions:
    for i in range(int(instruction["how_many"])):
        item = stacks[int(instruction["from"])-1][-1]
        stacks[int(instruction["to"])-1].append(item)
        stacks[int(instruction["from"])-1] = stacks[int(instruction["from"])-1][:-1]
