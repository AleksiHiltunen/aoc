...
while i < len(left):
    ...
    if left_small in range(right_small, right_big+1) or left_big in range(right_small, right_big+1):
        print("Left in range of right")
        result_pairs += 1
    elif right_small in range(left_small, left_big+1) or right_big in range(left_small, left_big+1):
